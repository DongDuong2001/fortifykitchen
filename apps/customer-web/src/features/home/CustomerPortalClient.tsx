...
export default function CustomerPortalClient({
  ...
  const [isLoadingMenu, setIsLoadingMenu] = React.useState(initialMenuItems.length === 0);
  ...
  // Client-side hydration/reload in case initial data was empty (e.g. fetch errors during build)
  React.useEffect(() => {
    ...
    if (initialSubscriptionPlans.length === 0) {
      setIsLoadingPlans(true);
      fetch(`${API_URL}/subscription-plans/public`)
        .then((res) => (res.ok ? res.json() : null))
        .then((plansData) => {
          if (plansData?.data) setSubscriptionPlans(plansData.data);
        })
        .catch(console.error)
        .finally(() => setIsLoadingPlans(false));
    }
    if (initialHomeFrames.length === 0) {
      setIsLoadingHomeFrames(true);
      fetch(`${API_URL}/home-frames`)
        .then((res) => (res.ok ? res.json() : null))
        .then((framesData) => {
          if (framesData?.data) {
            setHomeFrames(framesData.data.filter((f: any) => f.isActive));
          }
        })
        .catch(console.error)
        .finally(() => setIsLoadingHomeFrames(false));
    }
  }, [API_URL, initialMenuItems, initialSubscriptionPlans, initialHomeFrames]);
